/*
  ...
  Modified: Dic. 15, 2011 by Erica Vidal 
  Added type = (upf, txt)   // check extention of files UPF and txt
...
var msgUPF     = "must have UPF extention";
var msgTXT     = "must have txt extention";
...
function checkForm() {
  ...
  var valid_upf_file = /^[A-Za-z0-9._-]+\.(UPF|upf|xml)$/i;
   var valid_txt_file = /^[A-Za-z0-9._-]+\.(txt|TXT|in|IN|dat|DAT)$/i;
    ...
    for(i = 1; i < args.length; i++) {
      ...
      if(elem) {
        ...
        if(val != "") {
          ...
          else if(type == "upf" && val.search(valid_upf_file) == -1) msg += '"' + title + '" ' + msgUPF + "\n";
	  else if(type == "txt" && val.search(valid_txt_file) == -1) msg += '"' + title + '" ' + msgTXT + "\n";
