...
El sistema SEBREI integra la tecnología de sistema de experto con una interfaz web, utilizando
...
\end{figure}
 ...
 La memoria de trabajo también llamada base o lista de hechos (facts list)
...
para la producción de sistemas expertos. En \cite{shells} hay una extensa re- ferencia a distintas herramientas y shells 
...
Hacia mediados de la década del 80 el 95\% de los programas en IA estaban escritos en LISP. \cite{charniak}\footnote{pag. 33}
...
En las de inyección, lo que más se cambia son los cartuchos de negro
...
en donde suelen estar los tres colores básicos.La mayoría de las impresoras de nueva generación utilizan cartuchos
 individuales por cada color, esto permite al usuario reemplazar solo el color que se agote.
Además con la finalidad de mejorar los tonos claros y oscuros las nuevas impresoras fotográficas cuentan con seis
y hasta ocho colores diferentes (magenta claro, cyan claro, negro claro, azul marino, naranja, rojo y verde entre otros).
...
Usando como guía un lo manifestado por el experto y lo consultado en el resto de las fuentes ya mencionadas,
podemos establecer
...
\begin{tabular}{l}
 ...
 uso ( en qué ámbito será usado: oficina, hogar, etc)  \\
...
de reglas que luego deben ser validadas.
 ...
 \begin{quote}
	      ...
	      SI es fanático de la tecnología ENTONCES puede querer imprimir desde el iPod
...
SI se imprimen menos de 30 pag. a color  ENTONCES cartucho único.

SI se imprimen más de 30 pag a color  ENTONCES cartuchos separados.
...
SI se quieren imprimir fotografías ENTONCES no se recomienda una impresora domestica.
...
SI se requiere escanear en un estudio contable o de abogados ENTONCES conviene chequear la superficie del escáner admite
...
de acuerdo al uso que el cliente tiene intensiones de darle. En la tabla 1 
...
categorías y el ciclo mensual de trabajo, dicha relación se resume en la tabla 2.
...
En la figura 2 se presenta la arquitectura de tres niveles\footnote{
...
se utilizó un base de datos relacional para mantener las características de los equipos de impresión.  
Las interfaces web Cliente y Administrador son interfaces gráficas HTML embebidas en código de algún lenguaje 
de script. El SE se refiere al sistema experto que interactúa con el procesador de consulta y de anera indirecta
...
implementación del sistema se encuentran en el Apendice B.
...
La creación de hechos se realiza mediante las sentencias assert (uno solo hecho) o deffact (un conjunto), por ejemplo:
...
\end{figure}
\end{center}
El subárbol del nodo hogar se encuentra representado en la figura 5. 
...
o no, si sera necesario imprimir a 
...
\verb|adicionales.clp| es un conjunto de reglas adicionales relacionadas con características de los equipos que podrían ser necesarias
...
La regla default que selecciona los equipos que cumplen ciertas condiciones es la siguiente:
...
\verb|(member$ <single-field> <multifield>)|
Si el primer argumento es uno de los campos del segundo , la función retornará un entero con la posición del campo, 
de otro modo retornará FALSE.



\subsubsection{Reglas Adicionales}
Si se desea incluir la característica de sistema operativo entonces 
...
La regla adicionales incluye el assert

\verb|(assert (so any linux windows mac ))|

y a la regla recomendar se le debe agregar las condiciones correspondiente al sistema operativo. 
...
Dado que la regla adicionales  hace una assert de todos los sistemas operativos
 disponibles en la base de datos
y los equipos sólo tienen la posibilidad de tener asociados sistemas operativos
 que se encuentran en la base de datos en este punto haber seleccionado la característica
sistemas operativos para incorporar a las características de los equipos no tiene 
efecto alguno sobre las recomendaciones que el sistema está en grado de hacer. 
Es necesario modificar la regla de adicionales eliminando la claúsula 
(assert (so any linux windows mac )) y agregar una nueva regla que tome alguna decisión 
acerca de que sistema operativo debe soportar los euqipos a recomandar. 


\section{Evaluación del prototipo}

La regla selecciona todos los modelos de impresoras o multifuciones que cumplen
...
Dado que la cantidad de reglas es reducido y en general se las puede organizar
como árboles de desicion para las distintas características es fácil comprobar 
...
(watch <parámetro>): Es útil para depurar programas. El parámetro puede ser un de los siguientes símbolos: facts, rules, 
...
Se realizaron varios ensayos sobre el prototipo que cuenta con una lista de 45 equipos
 y con las distintas herramientas se corrigieron errores de sintácticos, 
de funciones, de tipeo y reglas faltantes. 

Ejemplo de un test de prueba.
Se elige un equipo de la base de conocimiento, en este caso el modelo
...
Luego se responden las pregunats hechas por el sistema de modo que si el sistema está correctamente implementado
...
LaserJet_Enterprise_M4555f_MFPn
\end{verbatim}\end{quote}  
...
encima del presupuesto que indicamos al sistema. A decir verdad un equipo con las características pedidas raramente tendrá
...
puede hacer es emitir un mensaje diciendo que todos los equipos superan el preuspuesto del cliente pero aún así recomendarlos. 
...
pueden observarse al tipear el comando facts:
...
Y la lista de matcheos, en particular el equipo recomendado corresponde al fact 37.
...
En la figura 6 se muestra el menú general del sistema. Cada link de la columna
...
El formulario de agregar un nuevo equipo (figura 7) sirve para incorporar a la base de datos, los datos correspondientes
...
categorías relacionadas con el precio de los equipos (Ver  figura 9).
...
segundo sirve de ejemplo para crear las reglas adicionales del archivo \verb|aditionals.clp|. (Ver figura 11). 
 \begin{figure}
 \centering
 \centering\setlength\fboxsep{0pt}
\setlength\fboxrule{0.5pt}
...
Se creo el siguiente deftemplate:
...
Dicha estructura se usa para compunicar la pregunta a la interfaz web. 
...
En todos los casos testeados la respuesta es la misma en ambas interfacez. En la figura 12 se encuentra el resultado
de dar las mismas respuestas que en el último test. 
La única diferencia entre la interfaz web y la interfaz para terminal es que en la interzas web además del modelos de los equipos
se da una breve descripción del modelo si es que está presente en la base de datos y un link al folleto o pagina web
del modelo. 

\newpage
\section{Conclusión y notas finales}
Construir el sistema ha servido para satisfacer mi curiosidad personal acerca de los sistemas expertos. 
En particular he comprendido que la parte mas difícil de la cosntrucción de este tipo de sistemas 
...
puede llegar a requerir varios años de desarrollo por lo que primero conviene hacer un verdadero estudio de la viabilidad del proyecto,
que por lo general incluye costo, disponibilidad de las fuentes del conocimiento, utilidad o necesidad de la construcción
del sistema, determinar si características del problema que resuelve son apropiadas para ser resuelto mediante un sistema
experto, entre otras. 
...
También es un buen atributo la posibilidad de agregar nuevas heurísticas sobre características presente en la base de datos
...
\appendix
\appendixname  \section{Reglas default}
...
SI se necesita una impresora movil y es necesario imprimir a color ENTONCES se recomienda tecnología 
de impresión a chorro de tinta.

SI se necesita una impresora movil y es necesario imprimir a color ENTONCES la tecnología 
...
SI el uso no es hogar ni profesionañ ENTONCES es una desición del cliente si quiere un equipos con
duplex automático.
...
SI el uso es profesional y el volumen mensual es hasta 800 páginas ENTONCES la tecnología de impresión puede ser laser o a chorro de tinta.
...
SI el uso es pyme+ y sólo se necesita imprimir ENTONCES se una impresora.
